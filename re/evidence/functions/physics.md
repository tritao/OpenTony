...
Status: confirmed dispatch/handler wiring; semantic labels cross-build correlated
...
`0x0049db80` is conservatively named `Skater_PhysicsDispatcher`; its case-0/case-1 routing is observed in the bounded Warehouse capture. `0x00497f40` is the runtime-confirmed in-air handler for raw state `1` and the statically shared handler for raw state `3`. The complete object layout and final semantic enum names remain unconfirmed.
