...
The live probe now ties both directional action edges to the shared handoff while distinguishing their current physics states. The Left sample reaches `0x0049917b`; the Right sample reaches `0x0049f0e5`. This makes the argument-building code immediately before both callsites the next useful instruction-level target for separating directional acceleration from state transitions.
...
- Repeat Left and Right while the skater is deliberately stationary on the ground, so both directions can be compared through `0x0049f0e5` without mixing physics states.
- Inspect the input-state writer around `0x00489966`/`0x0048996a` and the argument setup immediately before `0x0049917b` and `0x0049f0e5`.
