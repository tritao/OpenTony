...
`tony assets inspect-trg` reports the node-type histogram; add `--nodes` for offsets and node sizes, or `--scripts` for the bounded retail-PC autoexec, command-point, and restart-stream decoder. `tony assets inventory` summarizes the extracted tree by extension. The decoder preserves unknown opcodes as raw records because node payloads and command operands have type-specific structures and known THPS1/THPS2 variants.
...
The native reader in `src/assets/psx_collision.*` turns each blockmap's referenced object list into fixed-point world collision faces, applying the retail `object_position + vertex * 0x1000` placement, retaining object/model/face provenance, surface flags, and the packed face word at `+0x0c`, indexing faces by blockmap cell, and providing conservative segment queries. `PsxArchive::decode_texture()` expands the validated 4bpp/8bpp paletted and 16bpp payload forms into row-major RGB bytes for a future renderer. It intentionally does not assign surface-response or skater-radius semantics that have not yet been recovered.

The native reader in `src/assets/pre_asset.*` implements the inline PRE container used by `LEVEL.PRE`, `CREATE.PRE`, `PLAYER*.PRE`, and frontend packages. `PreAssetCatalog` provides case-normalized lazy loading over the extracted asset root; all 19 extracted PRE files currently pass the native parser.
The optional post-model tag header is an `u32` type followed by an `u32` payload size. The exact-build runtime recognizes `0x06` (`PK_TEXTUREWIBBLE`), `0x07` (`PK_COLOURPULSE`), `0x0a` (`PK_ZONE`, the blockmap), `0x2a` (`PK_3DANIM`), `0x2c` (`PK_COMPRESSED3DANIM`), `0x45` (`PK_VERTEXCOLOURS`), the little-endian `HIER` tag, and `RGBs`. Their runtime slot products and consumers are recorded in [psx-tags-runtime.md](../evidence/functions/psx-tags-runtime.md); `tony assets inspect-psx --tags` now reports these names instead of treating the recognized values as unknown.
