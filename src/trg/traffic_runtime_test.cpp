...
int main() {
    ...
    assert(list.regions().size() == 1);
    const auto& record = list.records().front();
    assert(record.source_node() == 0);
    assert(record.subtype() == 0xd5);
    assert(record.resource() == "c_taxi");
    assert(record.sound_id() == 0x71);
    assert(!record.sound_setup_disabled());
    assert(record.region_slot() == opentony::trg::kTrafficNoRegionSlot);
    assert(record.model_index() == 0);
    ...
    assert(record.constructor_parameters() == expected_parameters);
    assert(record.raw_record().size() == opentony::trg::kTrafficRuntimeRecordSize);
    assert(list.region(record.region_index()).resource == "c_taxi");
    assert(!list.region(record.region_index()).asset_available);
    assert(record.activation_argument() == 0);
    assert(record.activation_flags() == 0);
    assert(list.activate_node(0, 0x12345678U));
    assert(record.activation_argument() == 0x12345678U);
    assert((record.activation_flags() & 1U) != 0);
    assert(list.deactivate_node(0));
    assert((record.activation_flags() & 1U) == 0);
    assert(!list.activate_node(1, 0));
    const std::array<std::int32_t, 3> zero_response{0, 0, 0};
    assert(record.motion_response() == zero_response);
    assert(record.interaction_latch() == 0);
    ...
    assert(record.motion_response() == expected_response);
    assert(record.interaction_latch() == 1);

    opentony::trg::TrafficRuntimeList variants;
    variants.build(one_traffic_fixture(0xd6));
    assert(variants.records().front().resource() == "c_police");
    assert(variants.records().front().sound_setup_disabled());
    variants.build(one_traffic_fixture(0xdc));
    assert(variants.records().front().resource() == "c_gull");
    assert(variants.records().front().sound_id() == -1);
    assert(variants.record_for_node(0) != nullptr);
    assert(variants.record_for_node(1) == nullptr);
    ...
    if (std::filesystem::is_directory(asset_root)) {
        ...
        if (catalog.contains("c_taxi")) {
            assert(loaded.region(loaded_record.region_index()).asset_available);
            assert(loaded_record.region_slot() == 0);
            assert(loaded.region(loaded_record.region_index()).runtime.has_value());
            assert(loaded.region(loaded_record.region_index()).runtime->object_count() > 0);
        }
    }
    ...
    if (std::filesystem::is_regular_file(package_path)) {
        ...
        assert(packaged_record.resource() == "c_bull");
        if (packaged.region(packaged_record.region_index()).asset_available) {
            assert(packaged_record.region_slot() == 0);
            assert(packaged.region(packaged_record.region_index()).runtime.has_value());
            assert(packaged.region(packaged_record.region_index()).runtime->object_count() > 0);
        }
    }
