...
int main() {
    ...
    assert(falling.gravity_applied);
    assert(!falling.terminal_clamped);
    assert(falling.velocity == FixedPosition({100, -1500, 300}));

    // Equality is intentionally not the terminal branch in retail:
    // -0xe0c - 500 == -0x1000.
    const auto at_threshold = apply_air_gravity({100, -0xe0c, 300});
    assert(at_threshold.gravity_applied);
    assert(at_threshold.velocity == FixedPosition({100, -0x1000, 300}));

    const auto terminal = apply_air_gravity({100, -0xe0d, 300});
    assert(!terminal.gravity_applied);
    assert(terminal.terminal_clamped);
    assert(terminal.horizontal_reset);
    assert(terminal.velocity == FixedPosition({0, -0x1000, 0}));

    const auto custom = apply_air_gravity(
        {10, -20, 30},
        AirGravityConfig{4, -100, -50, false});
    assert(custom.velocity == FixedPosition({10, -24, 30}));
    ...
    const auto basis = opentony::runtime::rebuild_air_motion_basis(
        ...
        {0, 0x1000, 0});
    assert(basis.direction == FixedPosition({0, 0x1000, 0}));
    assert(basis.basis.at_3100 == FixedPosition({-0x1000, 0, 0}));
    assert(basis.basis.at_310c == FixedPosition({0, 0x1000, 0}));
    assert(basis.basis.at_30f4 == FixedPosition({0, 0, 0x1000}));

    using opentony::runtime::AirSpeedConfig;
    using opentony::runtime::compute_air_speed_scalar;
    assert(compute_air_speed_scalar(
    ...
    assert(up.delta == FixedPosition({0, 0, 150}));
    assert(up.motion_correction == FixedPosition({100, 200, 150}));
    assert(up.up_applied);
    assert(!up.down_applied);
    ...
    const auto down = opentony::runtime::apply_air_direction_input(
        ...
        true,
        opentony::runtime::AirDirectionInputConfig{100, 150});
    assert(down.motion_correction == FixedPosition({100, 200, 450}));
    ...
    const auto opposing = opentony::runtime::apply_air_direction_input(
        ...
        true,
        opentony::runtime::AirDirectionInputConfig{100, 150});
    assert(opposing.motion_correction == FixedPosition({100, 200, 300}));
