...
int main() {
    ...
    CHECK(quarter.at(0, 2) == -4095);
    CHECK(quarter.at(1, 1) == 0x1000);
    CHECK(quarter.at(2, 0) == 4095);
    CHECK(quarter.at(2, 2) == 0);
    ...
    CHECK(tiny.at(0, 0) == 4095);
    CHECK(tiny.at(0, 2) == 25);
    CHECK(tiny.at(2, 0) == -25);

    const auto turned = q12_apply_yaw(identity, 0x400);
    CHECK(turned.values == quarter.values);
    CHECK(q12_transform_vector(
        quarter,
        {0, 0, 0x1000})
        == opentony::runtime::FixedPosition({-4095, 0, 0}));

    const auto basis = opentony::runtime::retail_basis_from_matrix(quarter);
    CHECK(basis.at_30f4 == opentony::runtime::FixedPosition({-4095, 0, 0}));
    CHECK(basis.at_3100 == opentony::runtime::FixedPosition({0, 0, 4095}));
    CHECK(basis.at_310c == opentony::runtime::FixedPosition({0, 0x1000, 0}));
    ...
    CHECK(q12_apply_ground_yaw(saved_ground_matrix, -8).at(0, 2) == 44);
    CHECK(q12_apply_ground_yaw(saved_ground_matrix, -8).at(2, 0) == -45);
    const auto rotated_ground_velocity = q12_rotate_ground_velocity(
        {282, 0, 192408},
        saved_ground_matrix,
        -8);
    CHECK(rotated_ground_velocity
