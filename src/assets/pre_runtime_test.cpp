...
int main() {
    ...
    CHECK(manager.allocation_size() == 0x144);
    CHECK(manager.container_name(slot) == "PANEL.PRE");
    CHECK(manager.container(slot).entries().size() == 2);

    const auto image = manager.find_embedded("S2SWITCH.BMP");
    CHECK(image.has_value());
    CHECK(image->container_slot == 0);
    CHECK(image->container_name == "PANEL.PRE");
    CHECK(image->resource_name == "s2switch.bmp");
    CHECK(image->payload.size() == 3);
    CHECK(std::to_integer<std::uint8_t>(image->payload[2]) == 3);

    const auto nested = manager.find_embedded("dir/two.bin");
    CHECK(nested.has_value());
    CHECK(nested->payload.size() == 2);
    CHECK(std::to_integer<std::uint8_t>(nested->payload[1]) == 0xb);
    CHECK(!manager.find_embedded("missing.fnt").has_value());

    manager.unload("panel.pre");
    CHECK(manager.loaded_count() == 0);
    CHECK(!manager.find_embedded("s2switch.bmp").has_value());
    ...
    CHECK(manager.loaded_count() == opentony::assets::kRuntimePreSlotCount);
    rejected = false;
    try {
